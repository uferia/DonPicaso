...
## Final Whole-Branch Review Fixes (post-Task-15)

The final review (base `c2acee3`, head `156e867`) found three gaps beyond what any single task's own review caught — a cross-cutting review looking at the whole branch end-to-end. The user reviewed all three and approved fixing all of them before closing the branch:

1. **Login/StaffLogin timing-based account enumeration.** `LoginCommandHandler`/`StaffLoginCommandHandler` returned the same failure response body for "unknown email/PIN" and "wrong password/PIN", but only ran `passwordHasher.VerifyHashedPassword` when a user was actually found — so response latency leaked which emails/PINs exist. Fix: always call `VerifyHashedPassword` against a fixed dummy user + dummy hash when no real user is found, so both paths do equal work.

2. **Tenancy-scope enforcement was designed but never built.** The design spec (and this plan's Global Constraints, line 19) state "`Corporate` bypasses tenancy-scope checks entirely" — implying non-Corporate roles' access is checked against a resource's `brandId`/`branchId`. Task 4 built only role-rank checking (`RoleAuthorizationHandler` compares `(int)role <= (int)requirement.MinimumRole`); no scope dimension exists. This was not a bug in what Task 4 shipped — this sub-project's only protected endpoint (`/me`) returns the caller's own identity, so there was nothing to scope-check yet. Fix: extend `RoleRequirement`/`RoleAuthorizationHandler` with an optional `TenancyScope` (`None`/`Brand`/`Branch`) + route-parameter name, so a future endpoint can opt in via `RequireAuthorization(policy => policy.Requirements.Add(new RoleRequirement(UserRole.BranchManager, TenancyScope.Branch, "branchId")))`. Ship this as tested, working infrastructure — no endpoint in this branch consumes it yet (deliberately; the next sub-project's first resource-scoped endpoint is the first real consumer), so there's no route-level regression risk to check for beyond the new unit tests.

3. **Session doesn't survive a page reload on the POS tablet.** `AuthService.refresh()` already existed with a doc comment claiming it's "used on app start," but nothing actually called it there — `Program.ts`/`app.config.ts` never invoked it, and `roleGuard` only reads the in-memory `currentUser` signal, so any reload of `/admin` or `/pos` redirected to `/login` even with a valid refresh token in `localStorage`. This directly contradicted the design spec's promise that "a reload doesn't force a full re-login" (this plan's own Task 15 verification step, line 3979, had documented the broken behavior as "expected for this phase" — that framing is now superseded by this fix). Fix: wire `authService.refresh()` into a `provideAppInitializer` in `app.config.ts` so a stored refresh token is exchanged for a fresh access token before the app finishes bootstrapping.

All three are implemented in the commit(s) following `156e867`; see `.superpowers/sdd/progress.md` for the exact commit range and re-review outcome.
